...
/* internal functions */
static void binaryop(int op);
static void dumpvar(long *pool, int size);
...
void cycle(void)
{
  ...
  if (r_debug) {
    printf("\t\t\t\ttos %p: * %ld\n",
	   (void*)cur_robot->stackptr,*cur_robot->stackptr);
    printf("d,h,q,<cr>: ");
    if ((c=getchar()) == 'd') {
      printf("\nexternals");
...
static void dumpvar(long *pool, int size)
{
  ...
  for (i = 0; i < size; i++) {
    if (i % 3 == 0)
      printf("\n");
    printf("%8p: %8ld\t",(void*)(pool + i),*(pool + i));
  }
}
